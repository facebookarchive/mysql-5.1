...
static TABLE *my_open_table(XTThreadPtr self, XTDatabaseHPtr XT_UNUSED(db), XTPathStrPtr tab_path)
{
	...
	if ((error = open_table_def(*thd, share)) ||
		(error = open_table_from_share(thd, share, "", 0, (uint32_t) READ_ALL, 0, table, OTM_OPEN, TRUE)))
	{
		xt_free(self, table);
		lex_end(&new_lex);
		thd->lex = old_lex;
	...
	if ((error = open_table_from_share(thd, share, "", 0, (uint) READ_ALL, 0, table, OTM_OPEN, TRUE)))
#else
	if ((error = open_table_from_share(thd, share, "", 0, (uint) READ_ALL, 0, table, FALSE, TRUE)))
#endif
	{
		xt_free(self, table);
		lex_end(&new_lex);
