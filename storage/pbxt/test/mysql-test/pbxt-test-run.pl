...
sub main () {
  ...
  push(@opt_extra_mysqld_opt, "--default-storage-engine=pbxt");
  push(@opt_extra_mysqld_opt, "--binlog-format=mixed");

  open (FILE1, '../../mysql-src-root');
  my $mysql_path = <FILE1>;
  chomp ($mysql_path);
  $glob_basedir = "$mysql_path/sql";
  close (FILE1);

  command_line_setup();

  # PBXT: this is needed only for udf test that requires udf_example.dll to be present in the plugin dir
  if ( $glob_win32) 
  {
	  if (!$opt_extern) {
		push(@opt_extra_mysqld_opt, "--plugin_dir=".dirname($exe_mysqld));
	}
  }
  else
  {
    push(@opt_extra_mysqld_opt, "--plugin_dir=../../src/.libs");
  }
...
#  $glob_basedir= dirname($glob_mysql_test_dir);
  ...
  $master->[0]=
  {
   ...
   path_sock   => "$opt_vardir/run/master.sock",
  ...
  $master->[1]=
  {
   ...
   path_sock   => "$opt_vardir/run/master1.sock",
  ...
  $slave->[0]=
  {
   ...
   path_sock   => "$opt_vardir/run/slave.sock",
  ...
  $slave->[1]=
  {
   ...
   path_sock   => "$opt_vardir/run/slave1.sock",
  ...
  $slave->[2]=
  {
   ...
   path_sock   => "$opt_vardir/run/slave2.sock",
  ...
  $instance_manager=
  {
   ...
   path_sock =>       "$opt_vardir/run/im.sock",
  ...
  $instance_manager->{'instances'}->[0]=
  {
   ...
   path_sock    => "$opt_vardir/run/mysqld_1.sock",
  ...
  $instance_manager->{'instances'}->[1]=
  {
   ...
   path_sock    => "$opt_vardir/run/mysqld_2.sock",
  ...
  if ( $opt_extern )
  {
    ...
    if ($glob_win32) {
	$master->[0]->{'port'}=  3306;
	mtr_report("Using extern server at port '$master->[0]->{port}'");
    } else {
...
sub setup_vardir() {
  ...
  if ( ! $glob_win32 )
  {
    ...
    symlink("$glob_mysql_test_dir/std_data", "$opt_vardir/std_data");
  }
  else
  {
    ...
    mkpath("$opt_vardir/std_data");
    mtr_copy_dir("$glob_mysql_test_dir/std_data", "$opt_vardir/std_data");
}
...
sub install_db ($$) {
  ...
  # Add PBXT plugin
  mtr_tofile($bootstrap_sql_file,
	     "\nREPLACE INTO mysql.plugin VALUES ('PBXT', 'libpbxt.so');");
  mtr_tofile($bootstrap_sql_file,
	     "\nREPLACE INTO mysql.plugin VALUES ('PBXT_STATISTICS', 'libpbxt.so');");
...
sub mysqld_arguments ($$$$) {
  ...
  if ( $mysql_version_id >= 50036)
  {
    ...
    # doesn't work with MySQL 5.1.42
  ...
  {
    mtr_add_arg($args, "%s--open-files-limit=50000", $prefix);
  }
