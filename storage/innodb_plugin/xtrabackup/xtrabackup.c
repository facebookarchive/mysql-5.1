...
// dummy functions to handle admission control
void
...
void thd_change_transaction_count(void* thd, int delta)
{
	return;
}
...
void xtrabackup_stream_datafile(void);
my_bool xtrabackup_read_logfile(LSN64 from_lsn, my_bool is_last);
void track_updated_page(ulint, ulint);
...
char xtrabackup_real_stream_file_delta[FN_REFLEN] = "";
char *xtrabackup_stream_file_delta = xtrabackup_real_stream_file_delta;
char xtrabackup_real_stream_file_full[FN_REFLEN] = "";
char *xtrabackup_stream_file_full = xtrabackup_real_stream_file_full;
char *xtrabackup_pagetracker_lsn = NULL;
LSN64 pagetracker_lsn;
char xtrabackup_real_pagetracker_dir[FN_REFLEN] = "";
char *xtrabackup_pagetracker_dir = xtrabackup_real_pagetracker_dir;
my_bool xtrabackup_pagetracker_validate = FALSE;
LSN64 logcopy_startlsn = 0;

#define PAGETRACKER_VALIDATION_RESULT_MAX 6
int pagetracker_validation_results[1 + PAGETRACKER_VALIDATION_RESULT_MAX] = {0};
my_bool xtrabackup_target_dir_specified = FALSE;
...
uint parallel;
ibool check_parent_on_suspend = FALSE;

// Used if --pagetracker-lsn is set to track updated pages
PageTracker	pagetracker;	

// Used if --pagetracker-dir option is specified during restore
Bitmap		restore_bitmap;
LSN64		restore_bitmap_startlsn;
LSN64		restore_bitmap_endlsn;

// Used --pagetracker-dir option is specified during incremental backup
char pagetracker_dump_cmd_path[FN_REFLEN] = "";
char pagetracker_dump_data_path[FN_REFLEN] = "";
...
enum options_xtrabackup
{
  ...
  OPT_XTRA_STREAM_FILE_DELTA,
  OPT_XTRA_STREAM_FILE_FULL,
  OPT_XTRA_PAGETRACKER_LSN,
  OPT_XTRA_PAGETRACKER_DIR,
  OPT_XTRA_PAGETRACKER_VALIDATE,
  OPT_XTRA_BACKUP,
  OPT_XTRA_STATS,
  OPT_XTRA_PREPARE,
  OPT_XTRA_EXPORT,
  ...
  OPT_XTRA_PARALLEL,
  OPT_XTRA_CHECK_PARENT_ON_SUSPEND,
...
static struct my_option my_long_options[] =
{
  ...
  {"stream-file-delta", OPT_XTRA_STREAM_FILE_DELTA, "source file for incremental backup", 
   (G_PTR*) &xtrabackup_stream_file_delta,
   (G_PTR*) &xtrabackup_stream_file_delta, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
  {"stream-file-full", OPT_XTRA_STREAM_FILE_FULL, "source file for full backup", 
   (G_PTR*) &xtrabackup_stream_file_full,
   (G_PTR*) &xtrabackup_stream_file_full, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
  {"pagetracker-lsn", OPT_XTRA_PAGETRACKER_LSN, 
   "just track pages updated after pagetracker-lsn (if 0 use latest checkpoint)", 
   (G_PTR*) &xtrabackup_pagetracker_lsn,
   (G_PTR*) &xtrabackup_pagetracker_lsn, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
  {"pagetracker-dir", OPT_XTRA_PAGETRACKER_DIR, 
   "Use pagetracker running in this dir to optimize reads in incremental backup",
   (G_PTR*) &xtrabackup_pagetracker_dir,
   (G_PTR*) &xtrabackup_pagetracker_dir, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
  {"pagetracker-validate", OPT_XTRA_PAGETRACKER_VALIDATE, 
   "Check that all pages that incremental backup would dump are tracked by pagetracker",
   (G_PTR*) &xtrabackup_pagetracker_validate,
   (G_PTR*) &xtrabackup_pagetracker_validate, 
  ...
  {"check_parent_on_suspend", OPT_XTRA_CHECK_PARENT_ON_SUSPEND,
   "Check that parent process continues to exist when suspended and exit otherwise.",
   (G_PTR*) &check_parent_on_suspend,
   (G_PTR*) &check_parent_on_suspend, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
...
{
	ibool stream_file = (*xtrabackup_stream_file_delta || *xtrabackup_stream_file_full);
	fprintf((stream_file ? stderr : stdout) ,
		"%s  Ver %s Rev %s for %s %s (%s)\n" ,my_progname,
		XTRABACKUP_VERSION, XTRABACKUP_REVISION, MYSQL_SERVER_VERSION,
	       ...
	       char *argument)
{
  ...
  case OPT_XTRA_TARGET_DIR:
    ...
    xtrabackup_target_dir_specified = TRUE;
    break;
  case OPT_XTRA_STREAM_FILE_DELTA:
    strmake(xtrabackup_real_stream_file_delta,argument, sizeof(xtrabackup_real_stream_file_delta)-1);
    xtrabackup_stream_file_delta= xtrabackup_real_stream_file_delta;
    fprintf(stderr, "Xtrabackup called with stream-file-delta %s option\n", 
	    xtrabackup_stream_file_delta);
    break;
  case OPT_XTRA_STREAM_FILE_FULL:
    strmake(xtrabackup_real_stream_file_full,argument, sizeof(xtrabackup_real_stream_file_full)-1);
    xtrabackup_stream_file_full = xtrabackup_real_stream_file_full;
    fprintf(stderr, "Xtrabackup called with stream-file-full %s option\n", 
	    xtrabackup_stream_file_full);
    break;
  case OPT_XTRA_PAGETRACKER_DIR:
    strmake(xtrabackup_real_pagetracker_dir,argument, 
	    sizeof(xtrabackup_real_pagetracker_dir)-1);
    xtrabackup_pagetracker_dir= xtrabackup_real_pagetracker_dir;
    fprintf(stderr, "Xtrabackup called with pagetracker-dir %s option\n", 
	    xtrabackup_pagetracker_dir);
    break;
...
{
	fprintf(stderr, "xtrabackup: thd_merge_sort_block_size is called\n");
	return(1024*1024);
}
...
#else
	srv_use_fast_checksums = (ibool) innobase_fast_checksum;
#endif
...
xb_read_delta_metadata(const char *filepath, xb_delta_info_t *info)
{
	...
	fp = fopen(filepath, "r");
	if (!fp) {
	  fprintf(stderr, "Meta file %s can't be opened\n", filepath); 
	...
	fprintf(stderr, "page size from meta file is %lu\n", info->page_size);
...
// Keep track of counts of various chunk sizes
ulint xtrabackup_pagetracker_nchunks[COPY_CHUNK+1] = {0};

ulint ncalls_os_file_read = 0;

// Adjust chunk details if pagetracker bitmap is being used. Chunk is a
// consecutive bunch of bytes that we want to read in one call to os_file_read.
// Chunk should be as large as possible but <= COPY_CHUNK * page_size.
//
// If pagetracker bitmap IS NOT used, we read COPY_CHUNK pages in each call
// except when it runs over file size.
//
// If pagetracker bitmap IS used, we look for the first page in the chunk 
// that has bit set, say P1 (if it exists) and the last page < P1 + COPY_CHUNK,
// say P2 (of course P2 could be same as P1) and read all pages from P1 to P2
// in one call to os_file_read.
//
// Chunk details include
// (1) offset at which chunk starts (in/out) 
// (2) chunk size (in/out) 
// (3) nextoffset i/e., offset to resume at after the current chunk (in/out)
void
xtrabackup_adjustchunk(ulint page_size, IB_INT64 *offset, 
		       ulint *chunksize, IB_INT64 *nextoffset)
{
	ib_uint64_t startpage = *offset/page_size;

	// Note that endpage could go beyond file_size. That's
	// ok because Bitmap_* functions handle that.
	ib_uint64_t endpage = startpage + COPY_CHUNK;

	// get the first modified page with id >= startpage
	if (!Bitmap_getnext(&restore_bitmap, &startpage, 
			    endpage)) {
		*nextoffset = *offset + COPY_CHUNK * page_size;
		*chunksize = 0;
		*offset = *nextoffset;
		xtrabackup_pagetracker_nchunks[0]++;
		//fprintf(stderr, 
		//	"Pagetracker Chunk:start=%lu,end=%lu,size=%lu\n",
		//       startpage, endpage, 0);
		return;
	}

	*offset = startpage * page_size;
	endpage = startpage + COPY_CHUNK;
	*nextoffset = endpage * page_size;

	// find the last page (excluding endpage) within chunk that has bit set
	do {
		endpage--;

		ut_a(endpage >= startpage);
	} while (Bitmap_getbit(&restore_bitmap, endpage) <= 0);

	*chunksize = (ulint)((endpage - startpage + 1) * page_size);
	xtrabackup_pagetracker_nchunks[*chunksize/page_size]++;	


	//fprintf(stderr, "Pagetracker Chunk:start=%llu,end=%llu,size=%llu\n",
	//       *offset/page_size, *nextoffset/page_size, *chunksize/page_size);

}

// Validates the restore_bitmap (which was retrieved from pagetracker dump) 
// for a [space,pageno]
void
xtrabackup_pagetracker_validatebit(ulint space, ulint pageno, 
				   ibool newpage, LSN64 pagelsn)
{
	int	validation_result = -1; 
	ibool	pagetracked = (Bitmap_getbit(&restore_bitmap, pageno) > 0);

	if (!newpage && !pagetracked) {
		// valid case 1. Pagetracker did not track the page and
		// incremental backup would not have dumped the page. 
		validation_result = 1;
	} else if (newpage && pagetracked) {
		// valid case 2. Pagetracker tracked the page and
		// incremental backup would have dumped the page. 
		validation_result = 2;
	} else if (newpage && !pagetracked) {
		if (ut_dulint_cmp(restore_bitmap_endlsn, pagelsn) < 0) {
			// this page was modified after the lsn range 
			// of pagetracker
			validation_result = 3;
		} else {
			/*  
			Pagetracker did not track this page even though
			its LSN changed.
			Disabling these printfs because, unfortunately, 
			this is giving lot of false alarms.
			InnoDB's mtr with modification always flushes all 
			x_latched blocks by the mtr with LSN updating at 
			the commit of mtr, even if the mtr has not updated 
			all the blocks.
 
			From the code in buf_page_release() :
			...
			buf_flush_note_modification is the function that is 
			used to mark a buffer as dirty. So if at least one page 
			was modified in the group of pages that are X latched 
			as part of mtr, lsn of all those pages is updated and 
			they are marked dirty.

			fprintf(stderr, 
				"PageTracker ERROR for [%lu, %lu]\n",
				space, pageno);
			fprintf(stderr, 
				"Pagelsn=%llu, startlsn=%llu, endlsn=%llu\n",
				pagelsn, restore_bitmap_startlsn, 
				restore_bitmap_endlsn);
			*/	
			validation_result = 0;
		}
	} else if (!newpage && pagetracked) {
		// Pagetracker thinks this page was modified but pagelsn
		// is showing lower value. This could happen if the page
		// is still in buffer pool and was not flushed to disk.
		// For stats purpose, we classify these pages into 3 categories
		// as follows based on where pagelsn is relative to 
		// start of incremental backup (logcopy_startlsn) and
		// start of pagetracker itself (restore_bitmap_startlsn).
		if (ut_dulint_cmp(logcopy_startlsn, pagelsn) < 0) {
			validation_result = 4;
		} else if (ut_dulint_cmp(restore_bitmap_startlsn, pagelsn) < 0) {
			validation_result = 5;
		} else {
			validation_result = 6;
		}
	...
	if (validation_result >= 0 && 
	    validation_result <= PAGETRACKER_VALIDATION_RESULT_MAX) {
		pagetracker_validation_results[validation_result]++;
	} else {
		fprintf(stderr, "ERROR: PageTracker validation result %d\n",
			validation_result);
	}
}

// Requests a dump from pagetracker. All this means is to create a file
// PAGETRACKER_DUMP_CMD_FILE in the pagetracker-dir. This is called during
// incremental backups if pagetracker-dir option is set and 
// xtrabackup_stream_file_delta is not set. (xtrabackup_stream_file_delta
// is used just to stream delta of one file. In that case presumably
// xtrabackup is already called with stream option to do log copying and
// pagetracker dump would already have been requested.)
void
xtrabackup_request_pagetracker_dump()
{
	os_file_t	dump_cmd_file = -1;
	ibool		success = FALSE;
	ibool		exists = FALSE;
	int		retrycnt = 50;
	os_file_type_t  type;
	...
	ut_a(!*xtrabackup_stream_file_delta);

	if ((strlen(xtrabackup_pagetracker_dir) + 
	     strlen(PAGETRACKER_DUMP_DATA_FILE) + 1) >= FN_REFLEN) {
		fprintf(stderr, "ERROR: Path too long for pagetracker dump\n");
		return;
	} 

	sprintf(pagetracker_dump_cmd_path, "%s/%s", xtrabackup_pagetracker_dir, 
		PAGETRACKER_DUMP_CMD_FILE);


	// we need to create dump cmd file
	dump_cmd_file = os_file_create(pagetracker_dump_cmd_path, 
				       OS_FILE_OVERWRITE, OS_FILE_NORMAL, 
				       OS_DATA_FILE, &success);
	if (!success || (dump_cmd_file < 0)) {
		os_file_get_last_error(TRUE);
		fprintf(stderr, "ERROR: Could not request pagetracker dump\n");		
		return;
	}

	if (dump_cmd_file != -1) {
		os_file_close(dump_cmd_file);
	}

	sprintf(pagetracker_dump_data_path, "%s/%s", xtrabackup_pagetracker_dir, 
		PAGETRACKER_DUMP_DATA_FILE);

	// wait for the cmd file to disappear and data file to appear
	while (--retrycnt >= 0) {
		os_thread_sleep(200000); /*0.2 sec*/
		success = os_file_status(pagetracker_dump_cmd_path, 
					 &exists, &type);
		if (success && exists) {
			continue;
		}

		// Pagetracker seems to have removed the cmd file (which it
		// does after data file is written and flushed. 
		// Now check for data file.
		success = os_file_status(pagetracker_dump_data_path, 
					 &exists, &type);
		if (success && exists) {
			return;
		}
	...
	fprintf(stderr, "ERROR: No pagetracker dump file found\n");

}

// Reads logcopy_startlsn from xtrabackup_suspend file
ibool
xtrabackup_get_logcopy_startlsn() 
{
	...
	suspend_file = os_file_create(suspend_path, OS_FILE_OPEN,
				      OS_FILE_NORMAL, OS_LOG_FILE, 
				      &success);

	if (!success || (suspend_file < 0)) {
		os_file_get_last_error(TRUE);
		fprintf(stderr, "xtrabackup: 'xtrabackup_suspended' open error\n");
		return FALSE;
	}

	success = os_file_read(suspend_file, (void *)&logcopy_startlsn, 
			       (ulint)0, (ulint)0,
			       (ulint)(sizeof(LSN64)));

	if (!success) {
		os_file_get_last_error(TRUE);
		fprintf(stderr, "xtrabackup: 'xtrabackup_suspended' read error\n");
	} else {
		fprintf(stderr, "xtrabackup: locopy_startlsn is %llu\n", 
			logcopy_startlsn);
	}
...
// Reads the bitmap for the tablespace from pagetracker dump and does some
// basic checks to see if it is usable
ibool
xtrabackup_get_bitmap(ulint space) 
{
        if ((strlen(xtrabackup_pagetracker_dir) + 
	     strlen(PAGETRACKER_DUMP_DATA_FILE) + 1) >= FN_REFLEN) {
		fprintf(stderr, "ERROR: Path too long for pagetracker dump\n");
	...
	sprintf(pagetracker_dump_data_path, "%s/%s", 
		xtrabackup_pagetracker_dir,
		PAGETRACKER_DUMP_DATA_FILE);	

	if (!PageTracker_get_bitmap(pagetracker_dump_data_path,
				    space,
				    &restore_bitmap,
				    &restore_bitmap_startlsn,
				    &restore_bitmap_endlsn)) {
		return FALSE;
	}

	// We successfully read bitmap for the tablespace from pagetracker file.
	// However, we can only use it if
	// (a) pagetracker started at or before incremental-lsn
	// (b) pagetracker completed at or after log copying began
	// We do those checks below.
	if (ut_dulint_cmp(incremental_lsn, restore_bitmap_startlsn) < 0) {
		fprintf(stderr, "WARNING: Pagetracker startlsn %llu higher than %llu\n",
			restore_bitmap_startlsn, incremental_lsn);
		return FALSE;			
	}
	
	if ((logcopy_startlsn == 0) && !xtrabackup_get_logcopy_startlsn()) {
		return FALSE;
	}

	if (ut_dulint_cmp(restore_bitmap_endlsn, logcopy_startlsn) < 0) {
		fprintf(stderr, "WARNING: Pagetracker endlsn %llu lower than %llu\n",
			restore_bitmap_endlsn, logcopy_startlsn);
		return FALSE;
	}
	...
	fprintf(stderr,
		"INFO Pagetracker dump usable: %llu <= %llu and %llu <= %llu]\n",
		restore_bitmap_startlsn, incremental_lsn, logcopy_startlsn, 
		restore_bitmap_endlsn);
...
//	LSN64		flush_lsn;
	...
	ulint		page_size_shift;
 	ibool		usebitmap = FALSE;
	ibool		stream_file = (*xtrabackup_stream_file_delta || 
				       *xtrabackup_stream_file_full);
#ifdef INNODB_VERSION_SHORT
	ulint		zip_size;
#endif
...
skip_filter:
	...
	if (xtrabackup_incremental) {
		if (!*xtrabackup_stream_file_delta) {
			snprintf(meta_path, sizeof(meta_path),
				 "%s%s", dst_path, XB_DELTA_INFO_SUFFIX);
			strcat(dst_path, ".delta");
		} else {
			// if we are streaming delta of the table, put meta file 
			// as .meta in target_dir (where xtrabackup_suspended lives). 
			snprintf(meta_path, sizeof(meta_path),
				 "%s/%s", xtrabackup_target_dir, XB_DELTA_INFO_SUFFIX);
		}

		fprintf(stderr, "Writing meta info to %s\n", meta_path);
...
//	}
	...
	if ((xtrabackup_incremental && *xtrabackup_pagetracker_dir)) {
		usebitmap = xtrabackup_get_bitmap(node->space->id);
	}

	for (offset = 0; offset < file_size; offset = nextoffset) {
		ulint chunk;
		ulint chunk_offset;
		...
		// If we are close to end of file, refresh file size. 
		// Not sure if this is really needed though. 
		if (file_size <= offset + chunk) {
			file_size = os_file_get_size_as_iblonglong(src_file);
		}
		
		if (xtrabackup_incremental && usebitmap && 
		    !xtrabackup_pagetracker_validate) {
			xtrabackup_adjustchunk(page_size, &offset, &chunk,
					       &nextoffset);
			if (chunk == 0) {
				continue;
			}
...
read_retry:
	...
	if (page_in_buffer == page_size/4) {
		...
		if (!*xtrabackup_stream_file_delta) {
			...
			// os_file_write does not work with stdout. So use fwrite.
			ulint npages = fwrite(incremental_buffer, page_size, page_in_buffer, 
						stdout);
			if (npages != page_in_buffer) {
				fprintf(stderr, "Only %lu pages written instead of %lu\n",
						npages, page_in_buffer);
				goto error;
			}
                }
	...
	/* ========================================= */
				...
				if (usebitmap) {
					ulint   pageno = (offset + chunk_offset)/page_size;
					LSN64	pagelsn = MACH_READ_64(page + chunk_offset 
								       + FIL_PAGE_LSN);
					xtrabackup_pagetracker_validatebit(node->space->id,
									   pageno, newpage,
									   pagelsn);
				}
			}
			...
			if (!*xtrabackup_stream_file_full) {
				...
				// os_file_write does not work with stdout. So use fwrite.
				ulint nbytes = fwrite(page, 1, chunk, stdout);
				if (nbytes != chunk) {
					fprintf(stderr, "Only %lu bytes written instead of %lu\n",
						nbytes, chunk);
					goto error;
				}
			}
	...
	if (!stream_file) {
		...
		// os_file_flush does not work with stdout. So use fflush. 
		success = fflush(stdout);
		if (success != 0) {
			fprintf(stderr, "xtrabackup_copy_datafile failed to flush stdout\n");
			goto error;
		}
        }
...
//	LSN64		old_scanned_lsn;
	...
	if (xtrabackup_pagetracker_lsn) {
		// In this case we don't need to copy or write log files.
		// We just need to read log files to track updated pages. 
		return xtrabackup_read_logfile(from_lsn, is_last);
	}
...
// Modelled based on xtrabackup_copy_logfile. However xtrabackup_copy_logfile
// does not need to parse log records while this needs to. It actually passes
// callback function track_updated_page to the log record parsing function
// recv_scan_log_recs to track the updated pages.  
my_bool
xtrabackup_read_logfile(LSN64 from_lsn, my_bool is_last)
{
	/* definition from recv_recovery_from_checkpoint_start() */
	log_group_t*	group;
	//log_group_t*	up_to_date_group;
...
//		}
		...
		*/
		...
		ib_uint64_t lsn = log_copy_scanned_lsn;
		PageTracker_set_end_lsn(&pagetracker, lsn);
...
// This is the callback function that gets passed to recv_scan_log_recs
void
track_updated_page(ulint space, ulint page_no)
{
	// fprintf(stderr, "Tracking page [%lu, %lu]\n", space, page_no);
	PageTracker_track_page(&pagetracker, space, page_no);
}
...
ulint
#endif
log_copying_function(
	void*	arg)
{
	(void)arg;
	ulint	counter = 0;

	if (!xtrabackup_stream && !xtrabackup_pagetracker_lsn)
		ut_a(dst_log != -1);
	...
	while(log_copying) {
		...
		if(counter >= SLEEPING_PERIOD * 5) {
			int	dumpstatus = PAGETRACKER_NO_DUMP;
			...
			PageTracker_check_and_exit(&pagetracker);

			dumpstatus = PageTracker_check_and_dump(&pagetracker);

			if (dumpstatus == PAGETRACKER_NO_DUMP) {
				continue;
			}

			// we dumped - it could have succeeded/failed.
			// In this case we want to scan log file again
			// immediately without sleeping as we already
			// spent time dumping.
			if(xtrabackup_copy_logfile(log_copy_scanned_lsn, FALSE))
				goto end;
		}
...
"xtrabackup: remove old data files which contain your precious data!\n");
	...
	// if xtrabackup_stream_file_{delta,full} is specified, we just need to stream the
	// delta or full contents for one file. 
	if ((*xtrabackup_stream_file_delta) || (*xtrabackup_stream_file_full))  {
		xtrabackup_stream_datafile();
		return;
	} 
...
reread_log_header:
	...
	// We have found checkpoint
	logcopy_startlsn = checkpoint_lsn_start;

	if (xtrabackup_pagetracker_lsn) {
		printf("xtrabackup: Just tracking updated pages.\n");
		// In this case we don't need to run full recovery and so we 
		// don't through innodb_init(). We just initialize recv_sys
		// to enable us to parse log records. 
		recv_sys_create();
		recv_sys_init(0, TRUE);
		PageTracker_init(&pagetracker, xtrabackup_target_dir);

		if (pagetracker_lsn && 
		    (pagetracker_lsn != checkpoint_lsn_start)) {
			// We need to track pages updated since pagetracker_lsn.
			// It is not enough if we track pages modified since 
			// checkpoint_lsn_start as full backup may not have 
			// captured pages modified between the two checkpoints.
			// If we can validate that pagetracker_lsn is still 
			// valid, then we could start the scan from there. For
			// now we exit assuming this is rare case. 
			printf("xtrabackup: Abort due to new checkpoint %llu!",
			       checkpoint_lsn_start);
			checkpoint_lsn_start = pagetracker_lsn;			
			exit(1);
		}

		PageTracker_set_start_lsn(&pagetracker, checkpoint_lsn_start);
...
skip_logwrite:
	...
	if (xtrabackup_pagetracker_lsn) {
		PageTracker_create_pidfile(&pagetracker);
		// Since the only thing that pagetracker does is sniffing logs,
		// do it in this thread itself. Note that this function never
		// returns in this case (except for failures).
		log_copying_function(NULL);
		fprintf(stderr, "Pagetracker failed. log_copying_succeed is %d\n",
			(log_copying_succeed ? 1 : 0));
		exit(EXIT_FAILURE);
	} else {
		// Do the log copying in another thread
	...
	if (xtrabackup_suspend_at_end) {
		...
		// Put the checkpoint_lsn_start in the file
		success = os_file_write(suspend_path, suspend_file,
					&checkpoint_lsn_start,
					0, 0,
					sizeof(checkpoint_lsn_start));
		if (!success) {
			fprintf(stderr, "xtrabackup: Error: failed to write to 'xtrabackup_suspended'\n");
			// this is not fatal error and so we just move on
			success = TRUE;
		}
		...
		while (exists) {
			...
			if (check_parent_on_suspend && (getppid() == 1)) {
				// parent process was probably killed and so init
				// is now the parent
				fprintf(stderr, "Exiting assuming parent was killed\n");
				exit(EXIT_FAILURE);
			}
		}
...
"xtrabackup: export metadata of table '%s' to file `%s` (%lu indexes)\n",
					...
					while (index) {
						...
						printf(
"xtrabackup:     name=%s, id=%llu, page=%lu\n",
							index->name,
#if (MYSQL_VERSION_ID < 50500)
							index->id,
#else
...
// if xtrabackup_stream_file_delta is specified, we just need to stream the
// delta for one file. 
void
xtrabackup_stream_datafile(void) 
{
	char* path = NULL;
	char* xtrabackup_stream_deltaorfull = NULL;
	char* deltaorfull = NULL;
	char* last = NULL;
	char* lastbutone = NULL;
	char dbname[FN_REFLEN] = "";
	char* datafile = NULL;
	fil_system_t*   system = fil_system;
	fil_node_t* node = NULL;
	fil_space_t* space = NULL;
	fil_space_t* last_src = NULL;

	if (xtrabackup_incremental) {
		path = xtrabackup_stream_file_delta;
		xtrabackup_stream_deltaorfull = xtrabackup_stream_file_delta;
		deltaorfull = "delta";
	} else {
		path = xtrabackup_stream_file_full;
		xtrabackup_stream_deltaorfull = xtrabackup_stream_file_full;
		deltaorfull = "full";
	}
	...
	// Now last should have start of filename and lastbutone should have 
	// start of dbname
	datafile = last;
	if (datafile && !strcmp(datafile, "ibdata1")) {
		// system table space should already be opened. Do nothing.
		fprintf(stderr, "xtrabackup: Streaming ibdata1 %s\n", deltaorfull);
	} else if (!datafile || !lastbutone) {
		fprintf(stderr, "xtrabackup: Could not get dbname/filename from %s\n",
			xtrabackup_stream_deltaorfull);
		exit(1);
	} else {
		// copy over just dbname part
		int dbnamelen = last - lastbutone - 1;
		strncpy(dbname, lastbutone, dbnamelen);
		dbname[dbnamelen] = '\0';
		fprintf(stderr, "xtrabackup: Streaming db=%s,file=%s %s\n", 
			dbname, datafile, deltaorfull);
		fil_load_single_table_tablespace(dbname, datafile);
	}
	...
	while (space != NULL) {
		...
		while (node != NULL) {
			...
			if (!last || strcmp(last, datafile)) {
				node = UT_LIST_GET_NEXT(chain, node);
				continue;
			}

			/* copy the datafile */
			if(xtrabackup_copy_datafile(node, 1)) {
				fprintf(stderr, "Failed to stream %s\n",
					xtrabackup_stream_deltaorfull);
				exit(1);
			}

        		fprintf(stderr, "xtrabackup: xtrabackup_stream_datafile  completed %s\n",
                		xtrabackup_stream_deltaorfull);
			return;
		}
	...
	fprintf(stderr,"xtrabackup: Could not locate %s in space->chain\n" , 
		xtrabackup_stream_deltaorfull);
	exit(1);

}

// Parses string lsn into LSN64. Returns 0 if no error and 1 if error. 
int
xtrabackup_parse_lsn(char* lsnstr, LSN64 *lsn)
{
	...
	if (incremental_low) {
		*incremental_low = '\0';
		...
		lsn_high = strtoll(lsnstr, &endchar, 10);
		...
		*lsn = ut_dulint_create((ulint)lsn_high, (ulint)lsn_low);
	} else {
		error = 1;
	}
#else
	*lsn = strtoll(lsnstr, &endchar, 10);
	if (*endchar != '\0')
		error = 1;
#endif
	return error;
...
skip_tables_file_register:
...
#endif
	if (xtrabackup_pagetracker_lsn) {
		int error = xtrabackup_parse_lsn(xtrabackup_pagetracker_lsn,
						 &pagetracker_lsn);
		if (error) {
			fprintf(stderr, "xtrabackup: value '%s' may be wrong format for pagetracker-lsn option.\n",
				xtrabackup_pagetracker_lsn);
			exit(-1);
		}
	...
	if (xtrabackup_backup && xtrabackup_incremental) {
		...
		int error = xtrabackup_parse_lsn(xtrabackup_incremental, 
						 &incremental_lsn);
	...
	/* If xtrabackup_stream_file_delta is specified, then ... */
	if (*xtrabackup_stream_file_delta &&
		(!xtrabackup_backup || 		   // --backup must be specified
		!xtrabackup_target_dir_specified || // --target-dir has the suspended file
		!xtrabackup_incremental)) {	   // it must be incremental backup
		char* err = "stream-file-delta is only for incremental backup with target-dir";
		fprintf(stderr, "xtrabackup: error: %s\n", err);
		exit(-1);
	}

	/* If xtrabackup_stream_file_full is specified, then ... */
	if (*xtrabackup_stream_file_full &&
		(!xtrabackup_backup || 		   // --backup must be specified
		!xtrabackup_target_dir_specified || // --target-dir has the suspended file
		xtrabackup_incremental)) {	   // it must NOT be incremental backup
		char* err = "stream-file-full is only for full backup with target-dir";
		fprintf(stderr, "xtrabackup: error: %s\n", err);
		exit(-1);
	}

	/* If xtrabackup_pagetracker_lsn is specified, then ... */
	if (xtrabackup_pagetracker_lsn &&
	    (!xtrabackup_target_dir_specified || // target-dir not specified 
	     xtrabackup_incremental)) {	   // incremental specified
		char* err = "pagetracker-lsn with incremental or no target-dir";
		fprintf(stderr, "xtrabackup: invalid option mix : %s\n", err);
		exit(-1);
	}

	if (xtrabackup_pagetracker_validate &&
	    !*xtrabackup_pagetracker_dir) {
		char* err = "pagetracker-validate without pagetracker-dir";
		fprintf(stderr, "xtrabackup: invalid option mix : %s\n", err);
		exit(-1);
	}

	// If pagetracker-dir is specified and
	if (*xtrabackup_pagetracker_dir && 
	    (xtrabackup_pagetracker_lsn || // pagetracker-lsn is specified
	     !xtrabackup_backup ||	// not backup
	     !xtrabackup_incremental)) { // not incremental
		char* err = "pagetracker-dir is only for incremental backup";
		fprintf(stderr, "xtrabackup: invalid option mix : %s\n", err);
		exit(-1);
	}
	...
	if (!xtrabackup_stream) {
		print_version();
		if (xtrabackup_incremental) {
			FILE* file = (*xtrabackup_stream_file_delta ? stderr : stdout);
#ifndef INNODB_VERSION_SHORT
			fprintf(file, "incremental backup from %lu:%lu is enabled.\n",
				incremental_lsn.high, incremental_lsn.low);
#else
			fprintf(file, "incremental backup from %llu is enabled.\n",
				incremental_lsn);
#endif
		}
	} else {
	...
	/* cannot execute both for now */
	{
		...
		if (xtrabackup_pagetracker_lsn) num++;
	...
	/* --backup */
	if (xtrabackup_backup || xtrabackup_pagetracker_lsn) {
		xtrabackup_backup_func();

		if (xtrabackup_incremental && *xtrabackup_pagetracker_dir) {
			// note that this loop runs till COPY_CHUNK inclusive
			ulint totalcount = 0;
			ulint partialcount = 0;
			int lastprint = -1;
			int i;
			for (i=0; i <= COPY_CHUNK; i++) {
				totalcount += xtrabackup_pagetracker_nchunks[i];
				partialcount += xtrabackup_pagetracker_nchunks[i];

				// rather than printing count for every chunk size,
				// print counts for ranges of chunk sizes
				if ((i != COPY_CHUNK) && (i % 50 != 0)) {
					continue;
				}

				fprintf(stderr, "Count for chunk sizes %d-%d = %lu\n",
					lastprint + 1, i, partialcount);
				lastprint = i;
				partialcount = 0;
			}

			fprintf(stderr, "Total count = %lu\n", totalcount);
			fprintf(stderr, 
				"PageTracker validation results:%d,%d,%d,%d,%d,%d,%d\n",
				pagetracker_validation_results[0],
				pagetracker_validation_results[1],
				pagetracker_validation_results[2],
				pagetracker_validation_results[3],
				pagetracker_validation_results[4],
				pagetracker_validation_results[5],
				pagetracker_validation_results[6]);			
		}

		if (xtrabackup_incremental) {
			fprintf(stderr, "Number of calls to os_file_read = %lu\n",
				ncalls_os_file_read);
		}
	}
