	...
	mtr_t*				mtr)
{
	...
	byte* log_ptr = mlog_open_and_write_index(mtr, page, index, type,
	                                          compressed ? 1 : 0);
	if (!log_ptr)
		return;
	if (compressed) {
	...
	mtr_t*		mtr)	/*!< in: mtr */
{
	...
	if (UNIV_LIKELY_NULL(page_zip)
	    && UNIV_UNLIKELY(!page_zip_compress(compression_flags,
	                                        page_zip,
	                                        page,
	                                        index,
	                                        NULL))) {
