		...
		if (table_start &&
			...
			if (db_start &&
			   ...
			   (table_start - (db_start + 1)) < (FN_LEN+1)) {
				const char* partition_name_divider;
				const char* table_end;

				/* Success! */
				parsed = TRUE;

				partition_name_divider= strstr(table_start, "#P#");

				table_end = partition_name_divider ?
...
#define	FSP_FREE_LIMIT		12
		...
		if (!success) {
			fprintf(stderr, "InnoDB: Writing checksum to page 0 of %s failed\n", 
				filepath);  
			goto func_exit;
		}
		...
		if ((index == NULL) || (index->page != 3)) {
			fprintf(stderr, "InnoDB: 1st index on table %s is NULL or root is not 3\n", name);
			success = FALSE;
			goto func_exit;
		}
		...
		if (n_index != UT_LIST_GET_LEN(table->indexes)) {
			ulint n_index_from_dict = UT_LIST_GET_LEN(table->indexes);
		...
		for (i = 0; i < n_index; i++) {
			...
			if (strnlen(index_name, 1+MAX_TABLE_NAME_LEN) > MAX_TABLE_NAME_LEN) {
				...
				fprintf(stderr, 
					"InnoDB: Index name for table %s too long\n",
					table->name);
				success = FALSE;
				goto func_exit;
			}
			...
			if (dict_index == NULL) {
				fprintf(stderr, 
					"InnoDB: Index %s on table %s not found\n", 
					index_name, table->name);
				success = FALSE;
				goto func_exit;
			}

			new_id[i] = dict_index->id;
		...
		*/
...

		{
			...
			for (offset = 0; offset < free_limit_bytes;
				...
				if (!success) {
					fprintf(stderr, 
						"InnoDB : Read from tablespace file %s failed\n",
						filepath);				  
					goto func_exit;
				}
			...
			while (index) {
				...
				if (root_page[i] != index->page) {
					...
					if (error != DB_SUCCESS) {
						...
						if (error != DB_SUCCESS) {
							fprintf(stderr,
								"InnoDB: Error %lu in SYS_INDEXES update rollback\n",
								error);
						}
...
skip_check:
	...
	if (srv_expand_import && dict_table_flags_to_zip_size(flags)) {
		...
		for (;;) {
			...
			offsets = rec_get_offsets(node_ptr, index, offsets, ULINT_UNDEFINED, &heap2);
...
convert_err_exit:
			...
			if (!page_zip_compress(page_zip_compression_flags,
				...
				if (UNIV_UNLIKELY(page_no == dict_index_get_page(index))) {
					...
					node_ptr = dict_index_build_node_ptr(index, rec, new_page_no, heap2,
									     level);
				...
				offsets = rec_get_offsets(split_rec, index, offsets, n_uniq, &heap2);
