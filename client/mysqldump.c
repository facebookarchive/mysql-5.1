...
static ulong opt_compatible_mode= 0, opt_timeout=0;
...
static int connect_to_db(char *host, char *user,char *passwd)
{
  ...
  */
  ...
  if (opt_timeout)
  {
    my_snprintf(buff, sizeof(buff), "SET wait_timeout=%lu, net_write_timeout=%lu", opt_timeout, opt_timeout);
    if (mysql_query_with_error_report(mysql, 0, buff))
      DBUG_RETURN(1);
  }
...
{
  uint i;
  DBUG_ONLY my_bool body_found= 0;
...
static int start_transaction(MYSQL *mysql_con, char* filename_out, char* pos_out)
{
  MYSQL_RES *res = NULL;
  MYSQL_ROW row;
