                                 ...
                                 ulonglong* binlog_offset)
{
  ...
  info.binlog_offset = binlog_offset;
  info.error = true;
...
/** @brief
  ...
  When filep != null this can return the handler in filep, then the
  caller must call handler::delayed_drop_table and delete the handler.
*/
                    ...
                    handler **filep)
{
  ...
  if ((error= file->ha_delete_table(path, filep != NULL)) && generate_warning)
  {
    /*
...
/**
  ...
  @retval
    !0  Error
*/
int handler::delete_table(const char *name, my_bool delayed_drop)
{
  int saved_error= 0;
  int error= 0;
...
int
handler::ha_delete_table(const char *name, my_bool delayed_drop)
{
  mark_trx_read_write();

  return delete_table(name, delayed_drop);
}
...
*/
void handler::update_global_table_stats(THD *thd, uint keys_dirtied)
{
  ...
  if (table_stats)
  {
    ...
    if (table_stats->num_indexes && last_active_index != MAX_KEY)
    {
      ...
      */
      ...
      my_atomic_add_bigint(&(table_stats->indexes[ix].rows_inserted),
                           stats.rows_inserted);
      my_atomic_add_bigint(&(table_stats->indexes[ix].rows_updated),
                           stats.rows_updated);
      my_atomic_add_bigint(&(table_stats->indexes[ix].rows_deleted),
                           stats.rows_deleted);
      my_atomic_add_bigint(&(table_stats->indexes[ix].rows_read),
                           stats.rows_read);
      my_atomic_add_bigint(&(table_stats->indexes[ix].rows_requested),
      ...
      /*
        last_active_index is only set by index_init. If this instance is used for
        a table scan next then this prevents that work from being counted for an index.
      */
