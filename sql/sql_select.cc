				    ...
				    ulonglong options, THD *thd)
{
  ...
  create_info.internal_tmp_table= TRUE;
...
free_tmp_table(THD *thd, TABLE *entry)
{
  ...
  if (entry->file)
  {
    if (entry->db_stat)
      ...
      entry->file->ha_delete_table(entry->s->table_name.str, FALSE);
    delete entry->file;
  }
			     ...
			     int error, bool ignore_last_dupp_key_error)
{
 ...
 err1:
  new_table.file->ha_delete_table(new_table.s->table_name.str, FALSE);
 err2:
